package com.mima.feltwords.ui.theme

import androidx.compose.ui.graphics.Color

// 毛毡色板，自 iOS Theme.swift 平移（浅色 / 深色双主题）。

// 浅色
val FeltYellowLight = Color(0xFFFFD21F)   // 主背景
val FeltOrangeLight = Color(0xFFFF8A2A)   // 强调/按钮
val FeltCreamLight = Color(0xFFFFF6D8)    // 暖色卡片
val FeltMintLight = Color(0xFFA9EBD6)
val FeltSkyLight = Color(0xFFBDEEFF)
val FeltPinkLight = Color(0xFFFFB8C8)
val FeltInkLight = Color(0xFF3B2D1F)      // 主文字
val FeltSecondaryLight = Color(0xFF8D7A56)
val FeltSurfaceLight = Color(0xFFFFFFFF)

// 深色
val FeltYellowDark = Color(0xFF241D10)
val FeltOrangeDark = Color(0xFFF5963A)
val FeltCreamDark = Color(0xFF322B1E)
val FeltMintDark = Color(0xFF2C5147)
val FeltSkyDark = Color(0xFF223038)
val FeltPinkDark = Color(0xFF5E3540)
val FeltInkDark = Color(0xFFF6EAD0)
val FeltSecondaryDark = Color(0xFFB9A983)
val FeltSurfaceDark = Color(0xFF342C1F)
